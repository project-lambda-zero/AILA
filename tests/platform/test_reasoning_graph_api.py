...
from aila.modules.forensics.contracts import ReasoningGraphSnapshot
...
@pytest.mark.asyncio
async def test_list_reasoning_graphs_returns_snapshots(test_db) -> None:
    ...
    await ReasoningGraphService().save_snapshot(
        run_id="run-graph-1",
        module_id="forensics",
    ...
    handler = next(route.endpoint for route in router.routes if getattr(route, "path", "") == "/projects/{project_id}/investigations/{investigation_id}/reasoning-graphs")
    ...
    assert isinstance(snapshot, ReasoningGraphSnapshot)
    assert snapshot.step_number == 1
    assert snapshot.strategy_family == "filesystem_triage"
    assert snapshot.graph.nodes[0].id == "contract"
