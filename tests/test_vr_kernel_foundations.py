...
class TestPromptLoading:
    def test_kernel_audit_prompt_loads(self) -> None:
        text = _load_prompt("vulnerability_research.kernel_audit")
        assert "kernel audit" in text.lower()
        assert "slab" in text.lower()
        assert "refcount" in text.lower() or "ref-count" in text.lower()

    def test_hypervisor_audit_prompt_loads(self) -> None:
        text = _load_prompt("vulnerability_research.hypervisor_audit")
        assert "hypervisor" in text.lower()
        assert "virtio" in text.lower()
        assert "guest" in text.lower() and "host" in text.lower()

    def test_kernel_prompt_mentions_hardening(self) -> None:
        text = _load_prompt("vulnerability_research.kernel_audit")
    ...
    def test_hypervisor_prompt_mentions_iommu(self) -> None:
        text = _load_prompt("vulnerability_research.hypervisor_audit")
        assert "iommu" in text.lower()
        assert "dma" in text.lower()

    def test_unknown_kernel_strategy_falls_back_to_audit(self) -> None:
        # Loader strips dotted prefix; an unmapped suffix falls back to
        # the original system_audit.md
        text = _load_prompt("vulnerability_research.totally_unknown_kernel_thing")
