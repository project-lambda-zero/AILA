"""IDA middleware: coerce IDA auto-name strings on address kwargs.

The middleware's ``_coerce_ida_autoname_to_address`` module-level
helper rewrites agent-supplied IDA-style auto-names (``sub_474FC0``,
``loc_4012A0``, ``unk_402100``, ...) into ``0x<hex>`` strings before
dispatch. Without this step, MCP tools that declare an integer-shaped
address kwarg (``build_call_tree.root_address`` is the lead case)
reject the call with ``ValueError: invalid literal for int() with
base 16: 'sub_...'`` and the agent burns a turn re-asking with the
address it could have extracted from the name itself.

These tests pin the pattern matrix + the kwargs-name allowlist. The
helpers landed as module-level functions on
:mod:`aila.platform.mcp.middleware.ida` when RFC-11 Tier C collapsed
the bespoke ``IDABridgeTool`` onto the generic ``McpBridgeTool`` +
:class:`IdaMiddleware`.
"""
...
from aila.platform.mcp.middleware.ida import (
    ...
    _coerce_ida_autoname_to_address,
)
...
class TestIDAAutonameCoercion:
    def test_sub_prefix(self) -> None:
        out, notes = _coerce_ida_autoname_to_address(
            "build_call_tree",
    ...
    def test_every_known_prefix(self) -> None:
        # All 13 IDA auto-name prefixes the middleware recognizes.
        prefixes = [
    ...
    def test_lowercase_hex_tail(self) -> None:
        out, _ = _coerce_ida_autoname_to_address(
            "build_call_tree",
    ...
    def test_real_label_passes_through(self) -> None:
        # `_main` / `wmain` / `WinMain` / `main` -- the middleware doesn't
    ...
    def test_every_address_kwarg_name_coerced(self) -> None:
        # All 15 address-shaped kwarg names in the middleware's allowlist.
    ...
    def test_empty_kwargs(self) -> None:
        out, notes = _coerce_ida_autoname_to_address("x", {})
        assert out == {}
        assert notes == []
...
class TestEncodingValueCoercion:
    ...
    second-stage hunts on a test sample). The middleware now rewrites
    the alias forms to the canonical ``"utf16le"`` before dispatch.
    """

    def test_utf16_aliased_to_utf16le_on_list_strings(self) -> None:
        out, notes = _coerce_encoding_value(
            "list_strings",
    ...
    def test_unrelated_tool_untouched(self) -> None:
        ...
        # tool happens to take ``encoding``, the middleware must not
    ...
    def test_full_pipeline_normalizes_through_normalize_kwargs(self) -> None:
        ...
        mw = IdaMiddleware(spec=SERVER_SPECS["ida_headless"], module_id="vr")
        mw._known_params = {
            "list_strings": frozenset({"binary_id", "encoding", "section"}),
        }
        out, notes = mw._normalize_kwargs(
            "list_strings",
