...
* which MCP servers the platform knows about (audit-mcp, ida-headless)
...
# Static catalog. To add a new MCP, append here AND add a matching
# field to VRConfigSchema. The operator-facing UI auto-discovers from
# this list.
...
class McpRegistryService:
    ...
    async def probe_all(self) -> list[dict[str, Any]]:
        ...
        return list(await asyncio.gather(*(self._probe(s) for s in MCP_SERVERS)))
    ...
    async def update_base_url(self, server_id: str, base_url: str) -> dict[str, Any] | None:
        ...
        if spec is None:
            return None
        await self._registry.set("vr", spec["config_key"], base_url.rstrip("/"))
    ...
    async def _resolved_url(self, spec: dict[str, str]) -> tuple[str, str]:
        """Return (url, source). source ∈ {'env', 'config', 'default'}."""
        env_value = os.environ.get(spec["env_var"])
        if env_value:
            return env_value.rstrip("/"), "env"
        try:
            cfg_value = await self._registry.get("vr", spec["config_key"])
        except (ValueError, RuntimeError) as exc:
            _log.warning("ConfigRegistry get failed for vr/%s: %s", spec["config_key"], exc)
            cfg_value = None
