...
Generic branch-tree operations shared by every module. A module binds
its branch + investigation record models and its config namespace; the
platform owns the six branch transitions and the fork cap enforcement.

Historical note (M3.R-5):
...
class BranchPool:
    ...
    def __init__(
        ...
        branch_model: type[Any], investigation_model: type[Any],
        module_id: str,
    ) -> None:
        self.investigation_id = investigation_id
        self._branch_model = branch_model
        self._investigation_model = investigation_model
        self._module_id = module_id

    async def _max_branches(self) -> int:
        # Floor at 1 so a config typo cannot disable forking; the cap
        # itself is operator policy read live via ConfigRegistry.
        return max(
            1,
            await ModuleConfigReader(self._module_id).get_int(
    ...
    async def fork(
        ...
        *,
        ...
        async with UnitOfWork() as uow:
            # RFC-03 -- lock the investigation row first so concurrent
            # forks serialize on the cap check. The prior count-then-
            # insert was racy: two forks could both read active<cap
            # and both insert, exceeding the cap.
        ...
        *,
        ...
        async with UnitOfWork() as uow:
            ...
            siblings = (await uow.session.exec(
                _select(self._branch_model).where(
                    self._branch_model.investigation_id == self.investigation_id,
                    self._branch_model.id != branch_id,
                    self._branch_model.status.in_([
                        BranchStatus.ACTIVE.value, BranchStatus.PAUSED.value,
                    ]),
                ).with_for_update()
            )).all()
    ...
    async def list_active_by_strategy(
        ...
        async with UnitOfWork() as uow:
            rows = (await uow.session.exec(
                _select(self._branch_model)
                .where(
                    self._branch_model.investigation_id == self.investigation_id,
                    self._branch_model.status == BranchStatus.ACTIVE.value,
                )
                .order_by(self._branch_model.created_at.asc()),
            )).all()
    ...
    async def _load_branch(
        ...
        stmt = _select(self._branch_model).where(
            self._branch_model.id == branch_id,
        )
        if for_update:
    ...
    async def _load_parent_investigation(
        ...
        """Load the parent self._investigation_model for this manager (fix §64).
