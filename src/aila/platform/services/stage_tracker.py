...
Generic over the module: concrete subclasses bind the target record model
as a class variable, and module-level ``functools.partial`` wrappers bind
the model to the module-level helpers. The platform base owns the
idempotency / RUNNING-timeout / SELECT-FOR-UPDATE / optimistic-concurrency
logic and never names a module.
...
Semantics:
  ...
  - Entering the context loads the target row, inspects the stage's
    current status, and:
      * DONE      -> raises StageAlreadyDoneError (caller decides to skip)
      * RUNNING within stage_timeout_s -> raises StageInFlightError
...
_DEFAULT_TIMEOUTS: dict[StageName, float] = {
    ...
    # estimates; both are set to 30 min to avoid the reaper pre-empting a
    # decode that is still progressing. audit-mcp Trailmark + Semble build
    # over a 10k-class jadx Java tree can take 30-60 min (parse cache is
    # cold on the very first ingestion of each APK); the static summary
    # (manifest + signing + native + SBOM) is always under 1 min.
    StageName.APK_DECODE: 1800.0,
    StageName.JADX_DECOMPILE: 1800.0,
    StageName.REACT_NATIVE_EXTRACT: 1800.0,
    ...
    *,
    ...
    async with UnitOfWork() as uow:
        row = (await uow.session.exec(
            _select(target_model).where(target_model.id == target_id),
        )).first()
        if row is None:
...
class StageTracker:
    ...
    Generic over the module: concrete subclasses bind ``_target_model``
    to the module's target record model. See module docstring for
    usage. Construction does not touch the DB; entering the context
    does.
    """
        ...
        if stage_timeout_s is not None:
            ...
            if stage not in _DEFAULT_TIMEOUTS:
                raise RuntimeError(
                    ...
                    "add one to _DEFAULT_TIMEOUTS in platform/services/stage_tracker.py",
                )
    ...
    async def __aenter__(self) -> Self:
        ...
        # observes RUNNING (or DONE) and raises StageInFlightError instead of
        # both writing RUNNING and double-running the stage.
        async with UnitOfWork() as uow:
            row = (await uow.session.exec(
                _select(target_model)
                .where(target_model.id == self.target_id)
                .with_for_update(),
            )).first()
            if row is None:
            ...
            if current.state == StageState.RUNNING:
                ...
                # window? If yes, refuse to enter (StageInFlightError). If no,
    ...
    def record_output(self, **extra_columns: Any) -> None:
        ...
        Columns must exist on the bound target record model. Multiple
        calls before __aexit__ merge.
        """
    ...
    async with UnitOfWork() as uow:
        candidates = (await uow.session.exec(
            _select(target_model.id).where(
                ...
                target_model.analysis_state.in_([
    ...
    for target_id in candidates:
        try:
            async with UnitOfWork() as uow:
                ...
                upd_stmt = (
                    _update(target_model)
                    .where(target_model.id == target_id)
                    .where(target_model.analysis_stages_json == original_stages_json)
                    .values(**values)
                    .returning(target_model.id)
                )
