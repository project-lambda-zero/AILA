...
under the workspace artifact dir, then runs the 6-stage ingestion
pipeline (`APK_DECODE` → `JADX_DECOMPILE` → `REACT_NATIVE_EXTRACT` →
...
× L1 control count, e.g. `Run MASVS audit (~$2650)`). Click confirms,
...
1. **One parent investigation** with `kind=masvs_audit`,
   ...
   `cost_budget_usd=$2650`, `secondary_target_refs_json=[{
   masvs_spec_version: "1.4.2-aila" }]`.
2. **N child investigations** (N=53 for the current L1 catalog), each
...
ReportLab walks the ~53-control aggregate.
...
Per-control report subsections are generated by the dedicated `SectionWriterAgent` (`src/aila/modules/vr/reporting/section_writer.py`): one LLM call per control, run in parallel via `asyncio.gather`, cached on the outcome payload under the `_report_section` key. Replaces the prior dumb pass-through that dumped the auditor's free-text answer verbatim.
